...
function fish(shoal) {
  ...
  let eaten = 0;
  let amountToEat = 4;
  let shoalArr = shoal.split("");
  let hasEaten = true;
  while (hasEaten) {
    hasEaten = false;
    shoalArr.forEach((fishSize, i) => {
      const fishSizeInt = parseInt(fishSize)
      console.log(fishSizeInt)
      if (fishSizeInt <= mySize) {
        eaten += fishSizeInt
        hasEaten = true
        shoalArr.splice(i, 1)
        ...
        if (eaten >= amountToEat) {
          mySize++
          amountToEat += 4
          console.log("amountToEats " + amountToEat)
          eaten = 0
  ...
  console.log("final size " + mySize)
}

fish("7940900719626715817696801358463296590972287327396433567")
